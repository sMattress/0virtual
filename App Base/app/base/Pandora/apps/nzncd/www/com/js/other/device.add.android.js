...
window.DEVICE_ADD_ANDROID = (function() {
	...
	var _secondShow = function(t) {
		...
		mui('#device_add_iframe')[0].innerHTML = '<iframe id="wifiSet" height="190" src="http://10.10.11.234/wifi_set_cn.html"></iframe>' +
			'<iframe id="wifistatus" src="http://10.10.11.234/status_cn.html"></iframe>' +
			'<iframe id="site_survey" src="http://10.10.11.234/site_survey.html"></iframe>';
		var i = 0;
		...
		var t = setInterval(function() {
			if(i++ > 5) {
				...
				if(DEBUG && wifiSet) console.log('=正在获取信息====wifiSet 存在');
				wifistatus = document.getElementById('wifistatus').contentWindow;
				if(DEBUG && wifistatus) console.log('=正在获取信息====wifistatus 存在');
				site_survey = document.getElementById('site_survey').contentWindow;
				if(DEBUG && site_survey) console.log('=正在获取信息====site_survey 存在');
				if(wifiSet && wifistatus && site_survey) {
					_reGet = 0;
					clearInterval(t);
					_getForm();
				}
	...
	var _getForm = function(){
		var i = 0;
		var t = setInterval(function() {
			var $form = site_survey.document.getElementsByTagName('form')[0];
			if(DEBUG && $form) console.log('=$form已存在');
			if(i++ > 20) {
				clearInterval(t);
				COM.closeLoading();
				_endBack(true);
				...
				clearInterval(t);
				setTimeout(function(){
					mui('#hidden_wifi_list')[0].innerHTML = $form.innerHTML;
					_secondCreate();
				},3000);
			}
		},1000);
	...
	var _secondPwd = function(cSsid) {
		...
		tmpCssid = cSsid;
		mui('#ad_box>p')[0].innerHTML = '请输入 <span>' + cSsid + '</span> 的密码：';
	...
	var _secondSave = function() {
		if(DEBUG) console.log('=pwd====' + $pwd.value);
		if(wifiSet) {
			mui.confirm('请确认 ' + tmpCssid + ' 的wifi密码为 ' + $pwd.value +
				',如果wifi密码不正确,会导致床垫无法传递数据', '提示', CONSTANT.confirmBtn,
				function(e) {
					if(e.index == CONSTANT.confirmSure) {
						wifiSet.document.getElementById('stapw').value = $pwd.value;
...
//				}, 1000);
	...
	var _init = function() {
		...
		$pwd = document.getElementById('wifi_pwd');
	}
