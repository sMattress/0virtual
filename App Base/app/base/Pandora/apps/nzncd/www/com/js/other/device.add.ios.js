...
//				} else 
		...
		if(!connectTrue) {
			...
			var t = setInterval(function() {
				if(i++ < 40) {
					...
					mui.alert('无法获取到信息，请确认手机是否已经连入wifi', '提示');
					clearInterval(t);
					_endBack(true);
				}
			}, 3000);
	...
	var _goWifi = function() {
		if(DEBUG) console.log('==========执行  _goWifi');
	...
	var _goWifiAfter = function() {
		if(DEBUG) console.log('=====正在执行 _goWifiAfter');
