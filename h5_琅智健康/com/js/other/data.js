...
window.STORAGE = {
	...
	deviceUserInfoId:'deviceUserInfoId',//设备使用者信息绑定
	name: 'name', //用户姓名
	sex: 'sex', //用户性别  汉字
	sex_v: 'sex_v', //用户性别 标识
	...
	oneSleepId:'oneSleepId',//单个睡眠数据的id
	oneSleepDate:'',//单个睡眠数据的日期
	deviceListChange:'deviceListChange',//设备列表改变标识
};
...
URL.deviceBindUser = '/v1/user/device/bind_user';//绑定 设备的使用者信息
URL.deviceUpdateUser = '/v1/user/device/update_user';//更新 设备的使用者信息
URL.deviceGetUser = '/v1/user/device/get_user';//获取 设备的使用者信息
...
COMMON.menuClick = function() {
	...
	console.log('=====执行COMMON.menuClick;id===='+id)
	//COM.closeMark();
	if(NOW === COMMON) { //当前html属于com文件夹下边
		COM.openWindow(id,undefined,true);
	} else {
		COM.openWindow(id, '../../../com/html/other/' + id + '.html',true);
	}
}
...
YOUNG.model = 'young';
YOUNG.nav = {};
YOUNG.nav.links = ['monitor/young_monitor.html', 'record/young_record.html','user/young_user_info.html'];//, 'report/young_report.html'
YOUNG.nav.ids = ['young_monitor', 'young_record','young_user_info'];//, 'young_report'
YOUNG.nav.texts = ['睡眠状态', '睡眠记录','使用者信息'];//, '睡眠统计'
YOUNG.nav.icons = ['mui-icon mui-icon-gear', 'mui-icon mui-icon-paperplane','mui-icon mui-icon-person'];//, 'mui-icon-extra mui-icon-extra-trend'
YOUNG.nav.id = 'young_monitor';//当前显示的id
...
LILIAO.model = 'liliao';
LILIAO.nav = {};
LILIAO.nav.links = ['device/liliao_device.html', 'help/liliao_help.html'];//,'self/self.html'
LILIAO.nav.ids = ['liliao_device_list', 'liliao_help'];//,'self'
LILIAO.nav.texts = ['设备控制', '帮助'];//,'我'
