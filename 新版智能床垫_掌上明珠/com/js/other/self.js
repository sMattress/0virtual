SELF.start = function() {
	...
	COM.addHN(false, '我', CONST_MENU.home,true);
	COM.back();
...
SELF.getUserInfoSuccess = function(data) {
	if(data && data.flag) {
		...
		var imgUrl = "../../../com/img/other/LR_logo.png"; //头像url,,暂时没有
...
SELF.getUserInfo = function(success) {
	...
	console.log("SELF.getUserInfo 开始请求数据..");
	COM.ajax(obj);
}
		...
		if(id === "login_out") {
			mui.confirm('确认退出登录吗？', '提示', CONSTANT.confirmBtn, function(e) {
				if(e.index == CONSTANT.confirmSure) { //确定
					COM.clearStorage();
					COM.openWindow('LR', "../../../com/html/other/LR.html", true);
				}
			});
				...
				success: function(data) {
					...
					if(data && data.flag) {
						...
						if(latest.version_code > VERSION.value) {
							plus.ui.confirm(latest.version_name, function(rs) {
								if(rs.index == 0) {
			...
			COM.openWindow('../../../com/html/other/' + id);
		}
	});
}
