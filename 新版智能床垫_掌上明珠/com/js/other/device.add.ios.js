...
//				} else 
	...
	var _f12Show = function() {
		...
		if(wifiFlag === 1) {
			_nextFuc = _goWifi12;
			mui('#ad_box>p')[0].innerHTML = '点击下一步切换到<span>可用网络</span>；';
		} else {
			_nextFuc = _f12BindDo;
...
//				}, 1000);
	...
	var _goWifi12 = function() {
		...
		mui('#ad_box>p')[0].innerHTML = '切换到可用网络后，点击下一步保存数据；';
	}
	...
	var _goWifiAfter = function() {
		...
		$pwd.style.display = 'none';
		setTimeout(function() {
			mui('#ad_box>p')[0].innerHTML = '1.账号为带有<span>' + ssid + '</span>前缀的wifi名,密码:<span>' + pwd + '</span>;<br/>' +
	...
	var _reset = function() {
		...
		if(wifiFlag === 0) {
			...
			mui('#ad_box>p')[0].innerHTML = '1.请确认床垫<span>电源已打开</span>，wifi指示灯在闪烁；<br/>' +
				'2.手动连接床垫wifi，账号为带有<span>' + ssid + '</span>前缀的wifi名,密码:<span>' + pwd + '</span>;<br/>' +
