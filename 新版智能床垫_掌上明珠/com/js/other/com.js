...
COM.addReload = function(success) {
	if(DEBUG) console.log('COM.addReload 已执行');
...
COM.back = function(flag) {
	if(DEBUG) console.log('COM.back 已执行');
	var tag = 1;
...
COM.openWindow = function(id, url, flag) {
	...
	console.log('COM.openWindow已执行' + id);
	mui.openWindow({
		url: url,
		id: id,
		waiting: {
...
COM.addHN = function(back, title, menu, nav) {
	if(DEBUG) console.log('COM.addHN参数' + JSON.stringify(arguments));
...
COM.createMarkBody = function(list, sureFuc, cancelFuc) {
	console.log('COM.createMarkBody 已执行');
	COM.createMark();
	...
	for(var i = 0; i < list.length; i++) {
		...
		if(obj.type === 'input') { //输入框
			...
			for(var j = 0; j < child.length; j++) {
				...
				divDoc.innerHTML = '<span>' + childJ.text + '</span><img ' + hidden + ' src="../../img/other/right.png" />';
...
COM.countPosition = function(elem, type) {
	...
	if(type === 'fixed') {
		...
		if(DEBUG) {
			console.log('offsetTop:' + offsetTop + ';offsetHeight:' + offsetHeight);
		}
	}
	return obj;
}
...
COM.ajax = function(obj) {
	...
	if(DEBUG) console.log('请求数据网址：' + url);
	...
	try {
		mui.ajax(url, {
			...
			success: function(data, textStatus, xhr) {
				if(DEBUG) console.log('请求返回数据：'+JSON.stringify(data));
				if(!(data && data.flag)) {
					console.log(ERR_TEXT[data.err_code]);
					if(data.err_code != 64) {
			...
			error: function(xhr, type, errorThrown) {
				...
				if(type === 'timeout'){
					mui.alert('请求超时,请确认网络是否可用','提示');
				}else if(type === 'abort'){
					mui.alert('无法连接服务器,请确认是否已经连入网络','提示');
				}else{
					console.log('错误原因:' + type);
...
COM.getRandom = function(min, max) {
	if(max) {
		var t = parseInt((Math.random() * 1000)) % (max - min);
		return min + t;
	} else {
		max = min;
